...
`unhcrreports` automates the creation of humanitarian data reports by combining:
1.  **Data & Plots**: Using `unhcrviz` and `refugees` data.
2.  **AI Analysis**: Using LLMs (OpenAI, Azure, local models) to interpret plots.
3.  **Quarto**: To render beautiful, interactive or static HTML/PDF reports.
...
fusen::inflate(flat_file = "dev/flat_reports.Rmd", vignette_name = "Get Started", check = FALSE, overwrite = "yes")
pkgdown::build_site()
```
