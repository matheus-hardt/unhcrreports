---
title: "Regional Report Visualizations"
...
plot_reg_origin_dest <- function(year = 2022, region = "The Americas"){
  tryCatch({
    ...
    message("Error generating origin-destination plot: ", e$message)
    # Return empty plot or text on error to avoid breaking the report
    plot(1, type = "n", axes = FALSE, xlab = "", ylab = "")
    text(1, 1, paste("Plot generation failed:", e$message))
  })
...
plot_reg_increase <- function(year = 2021,
  ...
  p <- ggplot2::ggplot(
    ...
    unhcrthemes::theme_unhcr(font_family = "sans") +
    ggplot2::theme(
      panel.grid.major.x = ggplot2::element_line(color = "#cbcbcb"),
      panel.grid.major.y = ggplot2::element_blank()
    )
...
plot_reg_map <- function(year = 2022,
  ...
  regionname <- dplyr::case_when(
    ...
    region == "Asia and the Pacific" ~ "Asia & the Pacific",
  ...
  mapsf::mf_label(
    x = map_data[!is.na(map_data$value) & map_data$value >= minPopMap, ],
