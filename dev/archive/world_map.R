...
pop <- refugees::population |>
  ...
  filter(
    !(pop_type %in%
      c("hst", "stateless", "ooc", "returned_refugees", "returned_idps"))
  ) |>
  group_by(coa_iso) |>
...
# load vulnerability
vulnerability <- read_csv(here::here("boundaries", "vulnerability.csv")) |>
  janitor::clean_names() |>
...
# save plot
ggsave(
  "climate_vulnerability_vs_displaced_pop.png",
  device = agg_png,
  dpi = 300,
  width = 1280 * 2 / 300, # width of saved image
  height = 800 * 2 / 300, # height of saved image
